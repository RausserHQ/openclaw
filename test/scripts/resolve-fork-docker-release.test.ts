...
function resolveRelease(overrides: Record<string, string> = {}) {
  return resolveForkDockerRelease({
    ...
    revision,
    sourceRef: "refs/tags/v2026.7.1",
    sourceRefName: "v2026.7.1",
    ...overrides,
  });
}
...
describe("resolveForkDockerRelease", () => {
  it("publishes one immutable vSemVer image tag with OCI version and revision labels", () => {
    expect(resolveRelease()).toEqual({
      imageTag: "ghcr.io/rausserhq/openclaw:v2026.7.1",
      labels: [
  ...
  it("accepts SemVer prereleases when the package and tag match exactly", () => {
    const release = resolveRelease({
      packageVersion: "2026.7.2-alpha.1",
      sourceRef: "refs/tags/v2026.7.2-alpha.1",
      sourceRefName: "v2026.7.2-alpha.1",
    });

    expect(release.imageTag).toBe("ghcr.io/rausserhq/openclaw:v2026.7.2-alpha.1");
    expect(release.labels).toContain("org.opencontainers.image.version=2026.7.2-alpha.1");
  });

  it.each([
    ["main", "refs/heads/main", "main"],
    ["legacy fork tag", "refs/tags/rausser-2026.7.1", "rausser-2026.7.1"],
    ["mismatched version", "refs/tags/v2026.7.2", "v2026.7.2"],
  ])(
    ...
    (_case, sourceRef, sourceRefName) => {
      expect(() => resolveRelease({ sourceRef, sourceRefName })).toThrow(
        "Fork image publishing requires the exact release tag v2026.7.1",
      );
    },
  );
  ...
  it("keeps the workflow tag-gated and collision-safe", () => {
    ...
    expect(workflow).toContain("OPENCLAW_EXTENSIONS=slack");
